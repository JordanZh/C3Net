...
def create_dummy_control_class():
    dummy_class_item = dummy_class()
    dummy_class_item.input_blocks = []
    dummy_class_item.input_zero_convs = []
    for i in range(12):
        dummy_class_item.input_blocks.append(range(2))
        dummy_class_item.input_zero_convs.append(range(2))
    dummy_class_item.middle_block = range(3)
    dummy_class_item.middle_zero_block = range(3)
    return dummy_class_item  
...
class UNetModelCoDi(nn.Module):
    def __init__(self,
        ...
        self.control_scale = .3
        self.controlNet_uncond_prob = 0.0
        ...
        if 'video' in model_type or 'image' in model_type:
            ...
            control_unet_image_cfg = unet_image_cfg
            control_unet_image_cfg.type = 'control_unet_2d'
            self.control_unet_image = get_model()(control_unet_image_cfg)
        else:
            self.unet_image = create_dummy_class()
            self.control_unet_image = create_dummy_control_class()
            
        if 'text' in model_type:
            self.unet_text = get_model()(unet_text_cfg)
            self.text_model_channels = self.unet_text.model_channels
            control_unet_text_cfg = unet_text_cfg
            control_unet_text_cfg.type = 'control_unet_0dmd'
            self.control_unet_text = get_model()(control_unet_text_cfg)
        else:
            self.unet_text = create_dummy_class()
            self.control_unet_text = create_dummy_control_class()
                                             
        if 'audio' in model_type:
            self.unet_audio = get_model()(unet_audio_cfg)
            self.audio_model_channels = self.unet_audio.model_channels
            control_unet_audio_cfg = unet_audio_cfg
            control_unet_audio_cfg.type = 'control_unet_2d'
            self.control_unet_audio = get_model()(control_unet_audio_cfg)
        else:
            self.unet_audio = create_dummy_class()
            self.control_unet_audio = create_dummy_control_class()
    ...
    def forward(self, x, timesteps, condition, xtype, condition_types, x_0=[None], x_0_type='first_frame', mix_weight={'audio': 1, 'text': 1, 'image': 1}):
        ...
        if torch.rand(1).item() < self.controlNet_uncond_prob:
            context = self.the_uncondition
        else:
        ...
        for (i_module, t_module, a_module,\
            i_control_module, a_control_module, t_control_module,\
            i_zero, a_zero, t_zero,
            i_con_in, t_con_in, a_con_in) \
        in zip(
            self.unet_image.input_blocks, self.unet_text.input_blocks, self.unet_audio.input_blocks,
            self.control_unet_image.input_blocks, self.control_unet_audio.input_blocks, self.control_unet_text.input_blocks,
            self.control_unet_image.input_zero_convs, self.control_unet_audio.input_zero_convs, self.control_unet_text.input_zero_convs,
            ...
            h = [h_i for h_i in h]
            control = [None for _ in h]  # temp control for one iteration
            # Single type generation
            for i, xtype_i in enumerate(xtype):  # i: Enumerate type of output
                if xtype_i == 'audio':
                    for extra_condition in condition:  # For each type of condition
                        if control[i] is not None: control[i] += a_zero(a_control_module(h[i], emb_audio, extra_condition, x_0[i]))
                        else: control[i] = a_zero(a_control_module(h[i], emb_audio, extra_condition, x_0[i]))
                    with torch.no_grad():
                        h[i] = a_module(h[i], emb_audio, context, x_0[i])
                elif xtype_i in ['video', 'image']:
                    for extra_condition in condition:
                        if control[i] is not None: control[i] += i_zero(i_control_module(h[i], emb_image, extra_condition, x_0[i]))
                        else: control[i] = i_zero(i_control_module(h[i], emb_image, extra_condition, x_0[i]))
                    with torch.no_grad():
                        h[i] = i_module(h[i], emb_image, context, x_0[i])
                elif xtype_i == 'text':
                    for extra_condition in condition:
                        if control[i] is not None: control[i] += t_zero(t_control_module(h[i], emb_text, extra_condition, x_0[i]))
                        else: control[i] = t_zero(t_control_module(h[i], emb_text, extra_condition, x_0[i]))
                    with torch.no_grad():
        ...
        for i, xtype_i in enumerate(xtype):
            ...
            if xtype_i == 'audio':
                for extra_condition in condition:
                    if control[i] is not None: control[i] += self.control_unet_audio.middle_zero_block(
                            self.control_unet_audio.middle_block(h[i], emb_audio, extra_condition, x_0[i]))
                    else: control[i] = self.control_unet_audio.middle_zero_block(
                            self.control_unet_audio.middle_block(h[i], emb_audio, extra_condition, x_0[i]))
                with torch.no_grad():
                    h[i] = self.unet_audio.middle_block(h[i], emb_audio, context, x_0[i])
            elif xtype_i in ['video', 'image']:
                for extra_condition in condition:
                    if control[i] is not None: control[i] += self.control_unet_image.middle_zero_block(
                            self.control_unet_image.middle_block(h[i], emb_image, extra_condition, x_0[i]))
                    else: control[i] = self.control_unet_image.middle_zero_block(
                            self.control_unet_image.middle_block(h[i], emb_image, extra_condition, x_0[i]))
                with torch.no_grad():
                    h[i] = self.unet_image.middle_block(h[i], emb_image, context, x_0[i])
            elif xtype_i == 'text':
                for extra_condition in condition:
                    if control[i] is not None: control[i] += self.control_unet_text.middle_zero_block(
                            self.control_unet_text.middle_block(h[i], emb_text, extra_condition, x_0[i]))
                    else: control[i] = self.control_unet_text.middle_zero_block(
                            self.control_unet_text.middle_block(h[i], emb_text, extra_condition, x_0[i]))  
                with torch.no_grad():
            ...
            for h_ind in range(len(h)):
                h[h_ind] = h[h_ind] + control[h_ind] * self.control_scale
        ...
        in zip(
            ...
            temp = hs.pop()
            control_temp = control_stack.pop()
            # Single type generation
            for i, xtype_i in enumerate(xtype):
                h[i] = th.cat([h[i], temp[i] + self.control_scale * control_temp[i]], dim=1)
                if xtype_i == 'audio':
                    h[i] = a_module(h[i], emb_audio, context, x_0[i])
                elif xtype_i in ['video', 'image']:
                    h[i] = i_module(h[i], emb_image, context, x_0[i])
