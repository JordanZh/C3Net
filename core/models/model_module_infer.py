...
class model_module(pl.LightningModule):
    ...
    def inference(self, xtype=[], condition=[], condition_types=[], n_samples=1, mix_weight={'video': 1, 'audio': 1, 'text': 1, 'image': 1}, image_size=256, ddim_steps=50, scale=7.5, num_frames=8,
                  only_need_condition_list=False, only_latent_needed=False):
        net = self.net
        ...
        for i, condition_type in enumerate(condition_types):
            if condition_type == 'image':
                if not isinstance(condition[i], torch.Tensor):
        ...
        if only_need_condition_list:
            if not only_latent_needed:
                return conditioning
            else:
                ret1 = conditioning
        ...
        if only_latent_needed:
            if not only_need_condition_list:
                return z[0]
            else:
                return z[0], ret1
    ...
    def inference_(self, xtype=[], condition=[], condition_types=[], n_samples=1, mix_weight={'video': 1, 'audio': 1, 'text': 1, 'image': 1}, image_size=256, ddim_steps=50, scale=7.5, num_frames=8,
                    only_need_condition_list=False, only_latent_needed=False):
            net = self.net
            sampler = self.sampler
            ddim_eta = 0.0
            ...
            for i, condition_type in enumerate(condition_types):
                if condition_type == 'image':
                    if not isinstance(condition[i], torch.Tensor):
                        ctemp1 = regularize_image(condition[i], mode='test').cuda()
                    else:
                        ctemp1 = condition[i]
                    ctemp1 = ctemp1[None].repeat(n_samples, 1, 1, 1)
                    cim = net.clip_encode_vision(ctemp1).cuda()
                    ...
                    ctemp = condition[i].unsqueeze(0).repeat(n_samples, 1, 1)
                    cad = self.audio_encoder(ctemp)
                    cad = cad.unsqueeze(1)
                    uad = None
                    if scale != 1.0:
                        dummy = self.dummy_fbank.unsqueeze(0).repeat(n_samples, 1, 1)
                        uad = self.audio_encoder(dummy)
                        uad = uad.unsqueeze(1)
                    conditioning.append(torch.cat([uad, cad]))
