...
class UNetModelCoDi(nn.Module):
    def __init__(self,
        ...
        if 'video' in model_type or 'image' in model_type:
            ...
            # self.control_unet_image = create_dummy_class()
        ...
        if 'audio' in model_type:
            ...
            # self.control_unet_audio = create_dummy_class()
    ...
    def forward(self, x, timesteps, condition, xtype, condition_types, x_0=[None], x_0_type='first_frame', mix_weight={'audio': 1, 'text': 1, 'image': 1}):
        ...
        in zip(
            ...
            self.control_unet_image.input_blocks, self.control_unet_audio.input_blocks, self.control_unet_text.input_blocks, # TODO TODO TODO:no_grad()+no_optimizer
                ...
                if xtype_i == 'audio':
                    ...
                    for extra_condition in condition:
                        if control[i] is None:
                            #print(extra_condition.shape)
                            control[i] = a_zero(a_control_module(h[i], emb_audio, extra_condition, x_0[i]))
                        else:
                            control[i] += a_zero(a_control_module(h[i], emb_audio, extra_condition, x_0[i]))
                    with torch.no_grad():
                        #h[i] = a_module(h[i], emb_audio, condition[0], x_0[i])
                        h[i] = a_module(h[i], emb_audio, context, x_0[i])
                elif xtype_i in ['video', 'image']:
                    # h[i] = i_module(h[i], emb_image, context, x_0[i])
                    for extra_condition in condition:
                        if control[i] is None:
                            control[i] = i_zero(i_control_module(h[i], emb_image, extra_condition, x_0[i]))
                        else:
                            control[i] += i_zero(i_control_module(h[i], emb_image, extra_condition, x_0[i]))
                    with torch.no_grad():
                        #h[i] = i_module(h[i], emb_image, condition[0], x_0[i])
                        h[i] = i_module(h[i], emb_image, context, x_0[i])
                elif xtype_i == 'text':
                    for extra_condition in condition:
                        if control[i] is None:
                            control[i] = t_zero(t_control_module(h[i], emb_text, extra_condition, x_0[i]))
                            #print('control[i]: Need to be additive: ', type(control[i]))
                        else:
                            control[i] += t_zero(t_control_module(h[i], emb_text, extra_condition, x_0[i]))
                    with torch.no_grad():
                        #h[i] = t_module(h[i], emb_text, condition[0], x_0[i])
        ...
        #for i in range(len(control_stack)):
          #print(len(control_stack[i]))
          #print(control_stack[i][0].shape)
        ...
        for i, xtype_i in enumerate(xtype):
            ...
            if xtype_i == 'audio':
                ...
                for extra_condition in condition:
                    if control[i] is None:
                        control[i] = self.control_unet_audio.middle_zero_block(
                            self.control_unet_audio.middle_block(h[i], emb_audio, extra_condition, x_0[i]))
                    else:
                        control[i] += self.control_unet_audio.middle_zero_block(
                            self.control_unet_audio.middle_block(h[i], emb_audio, extra_condition, x_0[i]))
                with torch.no_grad():
                    #h[i] = self.unet_audio.middle_block(h[i], emb_audio, condition[0], x_0[i])
                    h[i] = self.unet_audio.middle_block(h[i], emb_audio, context, x_0[i])
            elif xtype_i in ['video', 'image']:
                # h[i] = self.unet_image.middle_block(h[i], emb_image, context, x_0[i])
                for extra_condition in condition:
                    if control[i] is None:
                        control[i] = self.control_unet_image.middle_zero_block(
                            self.control_unet_image.middle_block(h[i], emb_image, extra_condition, x_0[i]))
                    else:
                        control[i] += self.control_unet_image.middle_zero_block(
                            self.control_unet_image.middle_block(h[i], emb_image, extra_condition, x_0[i]))
                with torch.no_grad():
                    #h[i] = self.unet_image.middle_block(h[i], emb_image, condition[0], x_0[i])
                    h[i] = self.unet_image.middle_block(h[i], emb_image, context, x_0[i])
            elif xtype_i == 'text':
                for extra_condition in condition:
                    if control[i] is None:
                        control[i] = self.control_unet_text.middle_zero_block(
                            self.control_unet_text.middle_block(h[i], emb_text, extra_condition, x_0[i])
                        )
                        #print('control[i] middle: ', type(control[i]), control[i])
                    else:
                        control[i] += self.control_unet_text.middle_zero_block(
                            self.control_unet_text.middle_block(h[i], emb_text, extra_condition, x_0[i])
                        )
                with torch.no_grad():
                    #h[i] = self.unet_text.middle_block(h[i], emb_text, condition[0], x_0[i])
